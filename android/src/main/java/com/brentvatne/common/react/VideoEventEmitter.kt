...
class VideoEventEmitter {
    ...
    fun addEventEmitters(reactContext: ThemedReactContext, view: ReactExoplayerView) {
        ...
        if (dispatcher != null) {
            ...
            onVideoLoad = { duration, currentPosition, videoWidth, videoHeight, audioTracks, textTracks, videoTracks, manifest, trackId ->
                event.dispatch(EventTypes.EVENT_LOAD) {
                    putOnLoadData(duration, currentPosition, videoWidth, videoHeight, audioTracks, textTracks, videoTracks, manifest, trackId)
                }
            }
    ...
    private fun WritableMap.putOnLoadData(
        ...
        putString("trackId", trackId)
        putArray("videoTracks", Arguments.makeNativeArray(videoTracks.map { createVideoTrackInfo(it, false, manifest) }))
        putArray("audioTracks", Arguments.makeNativeArray(audioTracks.map { createAudioTrackInfo(it, false, manifest) }))
        putArray("textTracks", Arguments.makeNativeArray(textTracks.map { createTextTrackInfo(it, false) }))
